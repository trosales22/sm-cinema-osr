...
public class ShowAllAdministrators extends HttpServlet {
    Connection con = null;
    PreparedStatement pstmt = null;
    ResultSet rs;
    
    String fullName,emailAddress,accountType,dateAndTimeCreated,birthDay,userLoggedIn_type,action;
    JSONObject JSONAdministrators = new JSONObject();
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            pstmt = con.prepareStatement("SELECT * FROM TRISTANROSALES.TBLUSERSANDADMINS "
                    + "WHERE USER_TYPE != 'Customer' AND USER_TYPE != 'IT Administrator'");
            ...
            while(rs.next()){
                ...
                if(!userLoggedIn_type.equals("IT Administrator")){
                   ...
                   jsonObject.put("action", action); 
                    ...
                    action = "<a href='" + request.getContextPath() + "/ShowDetailsOfSelectedAdministrator?emailAddress=" + emailAddress + "'><button type='button' class='btn btn-danger'>Details</button></a>";
                    jsonObject.put("action", action);
                }
            ...
            JSONAdministrators.put("ADMINISTRATORS", jsonArray);
            response.getWriter().write(JSONAdministrators.toString());
        } catch (SQLException ex) {
            Logger.getLogger(ShowAllAdministrators.class.getName()).log(Level.SEVERE, null, ex);
        }finally{
            if(pstmt != null){
                try {
                    pstmt.close();
